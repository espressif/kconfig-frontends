...
envsubst:
  ...
  /* Is it a numeric parameter? */
    ...
    {
      /* Positional param not supported */
      value = NULL;
    }
  /* Is it a special parameter? */
	...
	{
      return WRDE_NOCMD; // Not supported
	}
      else
	{
	  /* Must be a quoted `$@' */
		...
		for (p = end; p >= value; --p)
		  {
			...
			*(char *) memcpy (newval, value, p - value) = '\0';
			if (free_value)
			  free (value);
			value = newval;
			free_value = 1;
